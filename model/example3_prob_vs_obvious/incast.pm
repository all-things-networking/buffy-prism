...
//  (all-uplink variant; auto-generated for MMAX=24 senders, 12 uplinks)
// =====================================================================
//
...
const int M;       // TOTAL SENDERS (input-traffic knob); even, 2..24; fan-out = M/2
...
const int MPP  = 2;
const int MMAX = 24;           // sender slots modelled (= MPP * 12 uplink ports)
...
formula a21 = (on21=1 & slot-t21<SLEN) ? 1 : 0;
formula a22 = (on22=1 & slot-t22<SLEN) ? 1 : 0;
formula a23 = (on23=1 & slot-t23<SLEN) ? 1 : 0;
formula a24 = (on24=1 & slot-t24<SLEN) ? 1 : 0;
...
formula arr11 = a21 + a22;
formula arr12 = a23 + a24;
...
formula f11 = (q11>0)?1:0;
formula f12 = (q12>0)?1:0;
formula Ftot = f1 + f2 + f3 + f4 + f5 + f6 + f7 + f8 + f9 + f10 + f11 + f12;
...
formula q11_next = (q11 - f11) + min(arr11, BUF - (q11 - f11));
formula q12_next = (q12 - f12) + min(arr12, BUF - (q12 - f12));
formula i_drop = (arr1 - min(arr1, BUF - (q1 - f1))) + (arr2 - min(arr2, BUF - (q2 - f2))) + (arr3 - min(arr3, BUF - (q3 - f3))) + (arr4 - min(arr4, BUF - (q4 - f4))) + (arr5 - min(arr5, BUF - (q5 - f5))) + (arr6 - min(arr6, BUF - (q6 - f6))) + (arr7 - min(arr7, BUF - (q7 - f7))) + (arr8 - min(arr8, BUF - (q8 - f8))) + (arr9 - min(arr9, BUF - (q9 - f9))) + (arr10 - min(arr10, BUF - (q10 - f10))) + (arr11 - min(arr11, BUF - (q11 - f11))) + (arr12 - min(arr12, BUF - (q12 - f12)));
...
module tor
    ...
    on21:[0..1] init 0; t21:[0..TWIN] init 0;
    on22:[0..1] init 0; t22:[0..TWIN] init 0;
    on23:[0..1] init 0; t23:[0..TWIN] init 0;
    on24:[0..1] init 0; t24:[0..TWIN] init 0;
    ...
    [] stage=13 & slot<HORIZON & 13<=M & on13=0 & slot<=WIN -> hz:(on13'=1)&(t13'=slot)&(stage'=14) + (1-hz):(stage'=14);
    [] stage=13 & slot<HORIZON & !(13<=M & on13=0 & slot<=WIN) -> (stage'=14);
    [] stage=14 & slot<HORIZON & 14<=M & on14=0 & slot<=WIN -> hz:(on14'=1)&(t14'=slot)&(stage'=15) + (1-hz):(stage'=15);
    [] stage=14 & slot<HORIZON & !(14<=M & on14=0 & slot<=WIN) -> (stage'=15);
    [] stage=15 & slot<HORIZON & 15<=M & on15=0 & slot<=WIN -> hz:(on15'=1)&(t15'=slot)&(stage'=16) + (1-hz):(stage'=16);
    [] stage=15 & slot<HORIZON & !(15<=M & on15=0 & slot<=WIN) -> (stage'=16);
    [] stage=16 & slot<HORIZON & 16<=M & on16=0 & slot<=WIN -> hz:(on16'=1)&(t16'=slot)&(stage'=17) + (1-hz):(stage'=17);
    [] stage=16 & slot<HORIZON & !(16<=M & on16=0 & slot<=WIN) -> (stage'=17);
    [] stage=17 & slot<HORIZON & 17<=M & on17=0 & slot<=WIN -> hz:(on17'=1)&(t17'=slot)&(stage'=18) + (1-hz):(stage'=18);
    [] stage=17 & slot<HORIZON & !(17<=M & on17=0 & slot<=WIN) -> (stage'=18);
    [] stage=18 & slot<HORIZON & 18<=M & on18=0 & slot<=WIN -> hz:(on18'=1)&(t18'=slot)&(stage'=19) + (1-hz):(stage'=19);
    [] stage=18 & slot<HORIZON & !(18<=M & on18=0 & slot<=WIN) -> (stage'=19);
    [] stage=19 & slot<HORIZON & 19<=M & on19=0 & slot<=WIN -> hz:(on19'=1)&(t19'=slot)&(stage'=20) + (1-hz):(stage'=20);
    [] stage=19 & slot<HORIZON & !(19<=M & on19=0 & slot<=WIN) -> (stage'=20);
    [] stage=20 & slot<HORIZON & 20<=M & on20=0 & slot<=WIN -> hz:(on20'=1)&(t20'=slot)&(stage'=21) + (1-hz):(stage'=21);
    [] stage=20 & slot<HORIZON & !(20<=M & on20=0 & slot<=WIN) -> (stage'=21);
    [] stage=21 & slot<HORIZON & 21<=M & on21=0 & slot<=WIN -> hz:(on21'=1)&(t21'=slot)&(stage'=22) + (1-hz):(stage'=22);
    [] stage=21 & slot<HORIZON & !(21<=M & on21=0 & slot<=WIN) -> (stage'=22);
    [] stage=22 & slot<HORIZON & 22<=M & on22=0 & slot<=WIN -> hz:(on22'=1)&(t22'=slot)&(stage'=23) + (1-hz):(stage'=23);
    [] stage=22 & slot<HORIZON & !(22<=M & on22=0 & slot<=WIN) -> (stage'=23);
    [] stage=23 & slot<HORIZON & 23<=M & on23=0 & slot<=WIN -> hz:(on23'=1)&(t23'=slot)&(stage'=24) + (1-hz):(stage'=24);
    [] stage=23 & slot<HORIZON & !(23<=M & on23=0 & slot<=WIN) -> (stage'=24);
    [] stage=24 & slot<HORIZON & 24<=M & on24=0 & slot<=WIN -> hz:(on24'=1)&(t24'=slot)&(stage'=25) + (1-hz):(stage'=25);
    [] stage=24 & slot<HORIZON & !(24<=M & on24=0 & slot<=WIN) -> (stage'=25);

    // substage MMAX+1: serve one slot, tally drops, advance the clock
    [step] stage=NSTAGE & slot<HORIZON ->
          (q1'=q1_next) & (q2'=q2_next) & (q3'=q3_next) & (q4'=q4_next) & (q5'=q5_next) & (q6'=q6_next) & (q7'=q7_next) & (q8'=q8_next) & (q9'=q9_next) & (q10'=q10_next) & (q11'=q11_next) & (q12'=q12_next)
        & (qo'=qo_next)
