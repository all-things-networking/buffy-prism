...
from src.netmdp.sampler.fixed_effort_smc_sampler import FixedEffortSMCSampler
...
def fixed_effort_cr_exp_p3(trial: int):
    ...
    sampler = FixedEffortSMCSampler("fqcodel_5i", prism_program, phi, 154,
                                    9, importance, user_effort, expected_success_effort)

    params = CROneHypParams(theta=0.00015, alpha=0.05, beta=0.05, zeta=0.0001, N=1, seed=1)
    results = run_cr_one_hyp(params, sampler)
    with open(f"fixed_effort_cr_p3_t{trial}.json", "w") as f:
        json.dump(results, f, indent=4)
    print(f"done fixed_effort_cr_p3_t{trial}")
...
def fixed_effort_cr_exp_p2(trial: int):
    path = "/Users/jkai/535C-project-smc/tests/prism_examples/fqcodel_5i_mdp.pm"
    prism_program = stormpy.parse_prism_program(path)
    ...
    expected_success_effort = 256
    sampler = FixedEffortSMCSampler("fqcodel_5i", prism_program, phi, 154,
                                    5, importance, user_effort, expected_success_effort)

    params = CROneHypParams(theta=0.011, alpha=0.05, beta=0.05, zeta=0.001, N=1, seed=1)
    results = run_cr_one_hyp(params, sampler)
    with open(f"fixed_effort_cr_p2_t{trial}.json", "w") as f:
        json.dump(results, f, indent=4)
    print(f"done fixed_effort_cr_p2_t{trial}")
    # print(f"Average stopping time: {results['stopping_times'].mean():.2f}")
    # print(f"Empirical type I error: {results['type_I']:.3f} (target = {params.alpha})")

def fixed_effort_cr_exp_p1():
    path = "/Users/jkai/535C-project-smc/tests/prism_examples/fqcodel_5i_mdp.pm"
    prism_program = stormpy.parse_prism_program(path)
    ...
    expected_success_effort = 256
    sampler = FixedEffortSMCSampler("fqcodel_5i", prism_program, phi, 154,
                                    3, importance, user_effort, expected_success_effort)

    params = CROneHypParams(theta=0.65, alpha=0.05, beta=0.05, zeta=0.05, N=24, seed=1)
    results = run_cr_one_hyp(params, sampler)
    with open(f"fixed_effort_cr_p1.json", "w") as f:
        json.dump(results, f, indent=4)
    print(f"done fixed_effort_cr_p1")
    # print(f"Average stopping time: {results['stopping_times'].mean():.2f}")
    # print(f"Empirical type I error: {results['type_I']:.3f} (target = {params.alpha})")

def experiment_fixed_effort_run():
    path = "/Users/jkai/535C-project-smc/tests/prism_examples/fqcodel_5i_mdp.pm"
    prism_program = stormpy.parse_prism_program(path)
    phi = "(true) U<=154 (iq5_deqs_bl>2)"
    importance = ImportanceFQCodel(prism_program, phi, 154, 14, 1)
...
if __name__ == "__main__":
    experiment_fixed_effort_run()
    # fixed_effort_cr_exp_p1(1)
    # fixed_effort_cr_exp_p2(1)
    # base_smc_cr_test()