...
def base_smc_cr_exp_p3(trial: int):
    ...
    with open(f"base_smc_cr_p3_t{trial}.json", "w") as f:
        json.dump(results, f, indent=4)
    print(f"done base_smc_cr_p3_t{trial}")
...
def base_smc_cr_exp_p2(trial: int):
    path = "/Users/jkai/535C-project-smc/tests/prism_examples/fqcodel_5i_mdp.pm"
    prism_program = stormpy.parse_prism_program(path)
    ...
    results = run_cr_one_hyp(params, sampler)
    with open(f"base_smc_cr_p2_t{trial}.json", "w") as f:
        json.dump(results, f, indent=4)
    print(f"done base_smc_cr_p2_t{trial}")
    # print(f"Average stopping time: {results['stopping_times'].mean():.2f}")
    # print(f"Empirical type I error: {results['type_I']:.3f} (target = {params.alpha})")

def base_smc_cr_exp_p1():
    path = "/Users/jkai/535C-project-smc/tests/prism_examples/fqcodel_5i_mdp.pm"
    prism_program = stormpy.parse_prism_program(path)
    ...
    results = run_cr_one_hyp(params, sampler)
    with open(f"base_smc_cr_p1.json", "w") as f:
        json.dump(results, f, indent=4)
    print(f"done base_smc_cr_p1")
    # print(f"Average stopping time: {results['stopping_times'].mean():.2f}")
    # print(f"Empirical type I error: {results['type_I']:.3f} (target = {params.alpha})")
