...
if __name__ == "__main__":
    BUF, THRESH = 32, 8
    # Example 2: short bursts, WIN >> SLEN (senders genuinely de-synchronised).
    # The least-favorable corner still has WIN/SLEN = 12 (spread over twelve
    # flow-lengths) yet P[Q] ~ 0.54.
    certify_box("Example 2 box A2 (desync / short bursts)",
                (16, 20), (48, 96), (8, 12), BUF, THRESH)
    print("\n# input buffers stay healthy across A2 (max P[input loss] over corners):")
    mx = max(Pin(0, M // 2, 2, S, W, BUF, THRESH)
             for M in (16, 20) for W in (48, 96) for S in (8, 12))
    print(f"  max P[input loss] = {mx:.3f}")
